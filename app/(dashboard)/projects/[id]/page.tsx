...
import type { Project, Column, Task } from "@/services/projectService";
...
export default function ProjectBoardPage({ params }: ProjectBoardPageProps) {
  ...
  const handleTaskSubmit = async (data: any) => {
    if (editingTask) {
      ...
      const updatedTask = await projectService.updateTask(editingTask.id, data);
      setProject(prev => {
        if (!prev) return prev;
        return {
          ...prev,
      ...
      // Create new task
      const newTask = await projectService.createTask(selectedColumnId, data);
      setProject(prev => {
        if (!prev) return prev;
        return {
          ...prev,
